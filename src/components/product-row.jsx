    ...
    <Grid container alignItems="center" style={{ padding: '16px 0' }}>
      ...
      >
        <Grid item xs={3} sm={4} className={horizontalCellClass}>
          <Typography variant={isWidthDown('xs', width) ? 'caption' : 'body2'}>
            {showPricing(price, moneyFormatter)}
          </Typography>
        </Grid>
        <Hidden smUp>
        ...
        <Grid
          ...
          xs={4}
          sm={5}
          className={horizontalCellClass}
          style={{ flex: 1 }}
        >
          <Typography variant={isWidthDown('xs', width) ? 'title' : 'body2'}>
            {moneyFormatter.format(calculatePrice(price, quantity))}
          </Typography>
        </Grid>
        <Hidden xsDown>
