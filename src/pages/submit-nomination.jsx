    ...
    if (!isServerSideRendering()) {
      ...
      try {
        const res = await request
          .post(`${process.env.GATSBY_API_URL}/nomination`)
          .send(values);
        if (res.ok) {
          ...
          this.sendSnackbarMsg(
            'error',
            'Failed to submit nomination, try again in a few minutes'
          );
        }
      } catch (e) {
        console.error(e);
    ...
    return (
      <div>
        ...
        <p className="p-margins">
          IEEE Elections: <br />
          March 13th: First day to apply for positions/nomination period begins{' '}
          <br />
        ...
        {!isLoading && (
          <Form inputs={inputs} onSubmit={values => this.onSubmit(values)} />
        )}
