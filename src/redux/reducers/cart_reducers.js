import { AddToCart, ClearCart, RemoveFromCart, UpdateInCart } from '../actions/types/cart_action_types';
...
function updateOrAdd(state, { id, name, imageURL, price, quantity, options }) {
  ...
  if (index > -1) {
    ...
    const optionIndex = item.options.findIndex(({ size, colour }) => size === options.size && colour === options.colour);
    if (optionIndex > -1) {
